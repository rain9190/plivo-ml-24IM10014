...
## Run 7 - Script-aware BPE with protected Hindi budget
- Hypothesis: plain BPE greedily takes the most frequent byte-pairs, and since English is 67 percent of the corpus, English pairs win almost every merge and Devanagari is starved, even though Hindi is 33 percent of the byte budget and costs 3 bytes per char where compression pays most. Measured on dev: English compressed to ~3.16 chars/token but Hindi to only ~1.58 chars/token. Protecting a Hindi merge budget should recover compression on the third of the data that byte-BPE neglects and lower bpb.
- Changed (vs Run 6): replaced plain BPE with script-aware BPE. First tried naive reallocation at vocab 2048, hindi_frac 0.45: Hindi improved to ~1.75 chars/token but English got worse because merges were taken from it, total tokens 46365 to 46442 (zero-sum, not shipped). Fixed by expanding the budget instead of stealing: vocab 2304 (untied, 1,995,200 params, under 2M cap), Devanagari given a protected pass at hindi_frac 0.42, English merges preserved. Model and recipe otherwise unchanged.
- dev bpb: 1.6947 -> 1.6933 (new best).
- Observation: total dev tokens 46365 to 45073 (down 2.8 percent). English held at ~3.2 chars/token, Hindi improved 1.58 to 1.74 chars/token. Lossless verified on train, dev, and arbitrary utf-8. The naive first attempt confirmed the reallocation is roughly zero-sum at a fixed budget; only expanding the budget under the cap gives a real gain.
- Conclusion: script-aware allocation recovers compression exactly where byte-BPE is weakest. The bpb gain is small but the mechanism is measured and sound. Overall vs baseline: 2.3718 -> 1.6933, a 28.6 percent reduction, within 2000 steps and under 2M params.
